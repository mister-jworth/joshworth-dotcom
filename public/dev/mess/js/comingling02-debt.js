...
function setup() {
  ...
  checkBars = createCheckbox('Bars', false);
  checkBars.position(width-400,10);
  checkBubbles = createCheckbox('Bubbles', false);
  checkBubbles.position(width-500,10);
  checkOrdering = createCheckbox('Reorder', false);
  ...
  checkSplitEffects.position(width-900,10);
}
...
class Citizen {
  ...
  displayCitizen() {
    ...
    if (checkBars.checked()){
        line(orderSpotAlt*(width/count)-10, height/2, orderSpotAlt*(width/count)-10, (height/2)-barHeight);
        //draw baselines
        if (checkBaseline.checked()){
         stroke(255,255,0)
         line(orderSpot*(width/count)-10,(height/2)-(this.wealthBase*scaleFactor)+2,orderSpot*(width/count)-10,(height/2)-(this.wealthBase*scaleFactor))
        }
    } 
    pop()
    ...
    //text(round(this.receiver.index, 3), this.vPosition.x, this.vPosition.y) //display wealthBase value at circle
};
}
