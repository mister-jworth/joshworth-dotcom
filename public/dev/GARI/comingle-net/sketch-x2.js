...
function draw() {
  ...
  if(millis() > cue){                         //after 1 second
    for (let i = 0; i < nodes.length; i++) {    //loop thru the nodes and
      if (cycles/nodes.length < 7){                //check if we've looped 7 times
       nodes[i].contribute();                       //if not then contribute
         if (fundsize < newfundsize){                 //and grow the fund to the new size
           fundsize += .1
         } else {
